...
while ( have_posts() ) :
    ...
    if ( empty( $accordion_items ) ) {
        if ( $legacy_description ) {
            $legacy_sections[] = array(
                ...
                'content' => apply_filters( 'the_content', $legacy_description ),
            );
        }
        ...
        if ( $legacy_benefits ) {
            $legacy_sections[] = array(
                ...
                'content' => apply_filters( 'the_content', wpautop( $legacy_benefits ) ),
            );
        }
        ...
        if ( $legacy_ritual ) {
            $legacy_sections[] = array(
                ...
                'content' => apply_filters( 'the_content', wpautop( $legacy_ritual ) ),
            );
        }

        if ( $legacy_ingredients ) {
            $legacy_sections[] = array(
                'title'   => __( 'Composición (INCI)', 'understrap' ),
                'content' => apply_filters( 'the_content', wpautop( $legacy_ingredients ) ),
            );
        }
        ...
        $accordion_items = $legacy_sections;
    }
    ?>
    ...
    <div id="product-<?php the_ID(); ?>" <?php wc_product_class( 'kapunka-single-product', $product ); ?>>
        ...
        <?php if ( ! empty( $accordion_items ) ) : ?>
            <div class="kapunka-clamp">
                <section class="product-accordion" id="kapunka-product-accordion">
                    <?php foreach ( $accordion_items as $index => $item ) :
                        $is_open = 0 === $index ? ' open' : '';
                        ?>
                        <details class="product-accordion__item"<?php echo $is_open; ?>>
                            <summary>
